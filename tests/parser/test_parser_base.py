...
class TestModelParser():
    ...
    def test_try_get_fail(self):
        with pytest.raises(ValueError) as exc:
            self.parser.__try_get(self.object_mock, 'feature')
            
        assert "Error in file ymal_mock.yaml: the field `feature` is required." in str(exc.value)
        assert exc.type == ValueError
        
    def test_get_fallback(self):
        response = self.parser.__get(self.object_mock, 'feature', 20)
        
        assert response == 20
        
    def test_get_good_flow(self):
        response = self.parser.__get(self.object_mock, 'data', 20)
